      ...
      <div class="grid gap-4 mb-2 md:grid-cols-1 w-full">
        <div class="grid gap-4 md:grid-cols-1 w-full">
          <div class="flex gap-3">
            <label class="font-semibold">Permission Name:</label>
            <div>{{ model.permission_name }}</div>
          </div>
          <div class="flex gap-3">
            <label class="font-semibold">Description:</label>
            <div>{{ model.permission_description }}</div>
          </div>
          <div class="flex gap-3">
            <label class="font-semibold">Divisions:</label>
            <div>
              {{
                model.divisions?.map(d => d.division_name).join(", ") || "None"
              }}
            </div>
          </div>
          <div class="flex gap-3">
            <label class="font-semibold">Positions:</label>
            <div>
              {{
                model.positions?.map(p => p.title).join(", ") || "None"
              }}
            </div>
          </div>
        </div>
