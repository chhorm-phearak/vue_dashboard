      ...
      <div class="grid gap-4 mb-2 md:grid-cols-2 w-full">
        <div class="grid gap-4 md:grid-cols-1 w-full">
          <div class="flex gap-3">
            <label class="font-semibold">Division Name:</label>
            <div>{{ model.division_name }}</div>
          </div>
          <div class="flex gap-3">
            <label class="font-semibold">Description:</label>
            <div>{{ model.division_description }}</div>
          </div>
        </div>
...
export default defineComponent({
  props: {
    ...
    editData: { type: Object, default: () => ({}) }, // pass selected division
  },
